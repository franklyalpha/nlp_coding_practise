...
def reward_to_go_calculation(gt_reward_stack, decay_factor=0.99):
    """

    :param gt_reward_stack: a 1D torch tensor where index "i" indicates the state reward at
        time "i", or at "ith" prompt.
    :param decay_factor: reinforcement learning long term reward decay factor
    :return: cumulative reward with same shape as gt_reward_stack
    """
    trajectory_time = gt_reward_stack.size()[0]
    reward_decay_tensor = torch.from_numpy(np.array([decay_factor ** i
                                                     for i in range(trajectory_time)]))
    reward_to_go = torch.zeros_like(gt_reward_stack)
...
preference_model = AbstractRewardModel(512, GENERATE_LIMIT)
...
critic_model = AbstractRewardModel(512, GENERATE_LIMIT)
# current_model.to(device)
